...
!
! DESCRIPTION: 
!> @Calculates langmuir hinchelwood rejection terms
!
!  Subroutine for Chemisorption 
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
! 
subroutine Chemisorption_LH_RT(J,Y,LH_RT)
  use global_variables
  implicit none
  ...
  ! Outputs
  real(double_precision), intent(out) :: LH_RT !< langmuir hinchelwood rejection term

  ! Locals
  REAL(double_precision) :: MLAY   !< Number of layers from which species can desorb
  REAL(double_precision) :: SUMLAY  !< Total number of layers on the grain surface (Chemisorption)
  REAL(double_precision) :: ab_surf !< Sum of all abundances on grain surface
  REAL(double_precision) :: ab_surf2 !< Sum of all abundances on grain surface (Chemisorption)
  ...
  real(double_precision) :: ab2H    !< H abundance on grains surface (Chemisorption)
  real(double_precision) :: abH    !< H abundance on grains surface 
  real(double_precision) :: ab2H2   !< H2 abundance on grains surface (Chemisorption)
  real(double_precision) :: ab2C    !< C abundance on grains surface (Chemisorption)
  real(double_precision) :: abC    !< C abundance on grains surface
  real(double_precision) :: ab2CH   !< C abundance on grains surface (Chemisorption)
  real(double_precision) :: abCH   !< C abundance on grains surface
  real(double_precision) :: ab2CH2  !< C abundance on grains surface (Chemisorption)
  real(double_precision) :: abCH2  !< C abundance on grains surface 
  real(double_precision) :: ab2CH3  !< C abundance on grains surface (Chemisorption)
  real(double_precision) :: abCH3  !< C abundance on grains surface
  real(double_precision) :: ab2CH4  !< CH4 abundance on grains surface (Chemisorption)
  real(double_precision) :: ab2C2H4 !< C2H4 abundance on grains surface (Chemisorption)
  real(double_precision) :: ab2CO   !< CO abundance on grains surface (Chemisorption)
  real(double_precision) :: ab2OH   !< OH abundance on grains surface (Chemisorption)
  real(double_precision) :: ab2O    !< O abundance on grains surface (Chemisorption)
  real(double_precision) :: ab2H2O  !< H2O abundance on grains surface (Chemisorption)
  ...
  DO I= nb_gaseous_species+1,nb_species
       IF (species_name(i)(1:1).eq."J") ab_surf = ab_surf + Y(i)
        IF (species_name(i)(1:1).eq."B") ab_surf2 = ab_surf+ab_surf2 + Y(i)
       ! PRINT *, species_name(J)
       IF(species_name(i).EQ.'JCO        ') abCO  = Y(i)
       IF(species_name(i).EQ.'JH2O       ') abH2O = Y(i)
       IF(species_name(i).EQ.'JNH3       ') abNH3 = Y(i)
       IF(species_name(i).EQ.'JCO2       ') abCO2 = Y(i)
       IF(species_name(i).EQ.'JCH4       ') abCH4 = Y(i)
       IF(species_name(i).EQ.'JCH3OH     ') abCH3OH = Y(i)
       IF(species_name(i).EQ.'BC         ') ab2C  = Y(i)
       IF(species_name(i).EQ.'JC         ') abC  = Y(i)
       IF(species_name(i).EQ.'BO         ') ab2O = Y(i)
       IF(species_name(i).EQ.'BH         ') ab2H  = Y(i)
       IF(species_name(i).EQ.'JH         ') abH  = Y(i)
       IF(species_name(i).EQ.'BH2        ') ab2H2 = Y(i)
       IF(species_name(i).EQ.'BCO        ') ab2CO = Y(i)
       IF(species_name(i).EQ.'BCH        ') ab2CH = Y(i)
       IF(species_name(i).EQ.'JCH        ') abCH = Y(i)
       IF(species_name(i).EQ.'BCH2       ') ab2CH2 = Y(i)
       IF(species_name(i).EQ.'JCH2       ') abCH2 = Y(i)
       IF(species_name(i).EQ.'BCH3       ') ab2CH3 = Y(i)
       IF(species_name(i).EQ.'JCH3       ') abCH3 = Y(i)
       IF(species_name(i).EQ.'BCH4       ') ab2CH4  = Y(i)
       IF(species_name(i).EQ.'BC2H4      ') ab2C2H4 = Y(i)
       IF(species_name(i).EQ.'BOH        ') ab2OH = Y(i)
       IF(species_name(i).EQ.'BH2O       ') ab2H2O = Y(i)
  enddo
 
  SUMLAY= ab_surf2*gtodn/nb_sites_per_grain
  MLAY= 1.0D+00
  !Calculating langmuir hinchelwood rejection terms
    !IF (REACTION_COMPOUNDS_NAMES(1,J).EQ.'CO         '.AND.&
     !   REACTION_COMPOUNDS_NAMES(4,J).EQ.'BCO        ') THEN
      !  MLAY = 0.5D+00
    !ELSE
     !   MLAY = 1.0D+00
    !END IF
    IF (REACTION_COMPOUNDS_NAMES(1,J).EQ.'H2         '.AND.&
        REACTION_COMPOUNDS_NAMES(4,J).EQ.'BH         ') THEN
        LH_RT=MLAY-(ab2H+abH)*MLAY*gtodn/nb_sites_per_grain
    ELSEIF (REACTION_COMPOUNDS_NAMES(1,J).EQ.'CO         '.AND.&
             REACTION_COMPOUNDS_NAMES(4,J).EQ.'BCO        ') THEN
             LH_RT=MLAY-((ab2CO+abCO+ab2C+abC+ab2CH+abCH+ab2CH2+abCH2+ab2CH3+abCH3)&
                   *MLAY*gtodn/nb_sites_per_grain)
    ELSE
        LH_RT=1.0D+00
    ENDIF
        LH_RT=  LH_RT*(1.0D+00-SUMLAY/MLAY)

    ! PRINT *, REACTION_COMPOUNDS_NAMES(1,J), REACTION_COMPOUNDS_NAMES(4,J)  
    !PRINT *, "LH_RT(J) for reaction:",J,'is=',LH_RT 
    !PRINT *, "MLAY for reaction:",J,'is=',MLAY
    !PRINT *, "SUMLAY for reaction:",J,'is=',SUMLAY
  ...
  return
  end subroutine Chemisorption_LH_RT
...
! If ((REACTION_COMPOUNDS_NAMES(1,J).EQ.'J          ')) then
  ...
  do J=type_id_start(88),type_id_stop(88)
    ...
    ! adsorption
    ACCRETION_RATES(reactant_2_idx(J)) = ACCRETION_RATES(reactant_1_idx(J))
    CALL Chemisorption_LH_RT(J,Y,LH_RT)
    reaction_rates(J) = RATE_A(J) * LH_RT*branching_ratio(J) * ACCRETION_RATES(reactant_1_idx(J)) / Y(reactant_1_idx(J)) / GTODN
...
! If ((REACTION_COMPOUNDS_NAMES(1,J).EQ.'J          ')) then
  ...
  do J=type_id_start(24),type_id_stop(24)
    reaction_rates(J) = RATE_A(J) * EXP(-RATE_C(J) * actual_av) * UV_FLUX
   !PRINT *, REACTION_COMPOUNDS_NAMES(1,J)
   !PRINT *, "The reaction rate(J) (ITYPE 24) for reaction:",J,'is=',reaction_rates(J)su
  enddo
 ...
 ! do J=type_id_start(29),type_id_stop(29)
   ! reaction_rates(J)=RATE_A(J)*LH_RT(J)*VIBRATION_FREQUENCY(reactant_1_idx(J))*EXP(-DISSOCIATION_ENERGY(reactant_1_idx(J))/&
    !                  actual_dust_temp)
      !PRINT *, REACTION_COMPOUNDS_NAMES(1,J), REACTION_COMPOUNDS_NAMES(2,J) 
...
!print *, 'cond=', cond
   ...
   IF (actual_gas_temp .GE. 100) THEN 
